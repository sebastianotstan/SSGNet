...
if __name__ == '__main__':
    ...
    parser.add_argument('--loss', '-lo', type=str, choices=['tvmf','Atvmf'], default='tvmf')
    ...
    # save #
    # if not os.path.exists("{}_{}_{}".format(args.out, args.model, args.cross)):
    #   	os.mkdir("{}_{}_{}".format(args.out, args.model, args.cross))
    # if not os.path.exists(os.path.join("{}_{}_{}".format(args.out, args.model, args.cross), "model")):
    #   	os.mkdir(os.path.join("{}_{}_{}".format(args.out, args.model, args.cross), "model"))

    run_name = f"{args.out}_{args.model}_{args.loss}_{args.cross}_s{args.seed}"
    os.makedirs(run_name, exist_ok=True)
    os.makedirs(os.path.join(run_name, "model"), exist_ok=True)

    # PATH_1 = "{}_{}_{}/trainloss.txt".format(args.out, args.model, args.cross)
    # PATH_2 = "{}_{}_{}/testloss.txt".format(args.out, args.model, args.cross)
    # PATH_3 = "{}_{}_{}/DSC.txt".format(args.out, args.model, args.cross)

    PATH_1 = f"{run_name}/trainloss.txt"
    PATH_2 = f"{run_name}/testloss.txt"
    PATH_3 = f"{run_name}/DSC.txt"
    ...
    cfg_path = os.path.join(run_name, "model", "config.json")
    with open(cfg_path, "w") as f:
        json.dump(cfg, f, indent=2, sort_keys=True)

    with open(os.path.join(run_name, "model", "loss.txt"), "w") as f:
        f.write(args.loss + "\n")
    ...
    train_loader = torch.utils.data.DataLoader(data_train, batch_size=args.batchsize, shuffle=True, drop_last=True, num_workers=0)
    val_loader = torch.utils.data.DataLoader(data_val, batch_size=args.batchsize, shuffle=False, drop_last=True, num_workers=0)
    ...
    for epoch in range(args.num_epochs):
        ...
        # PATH_train ="{}_{}_{}/model/model_last.pth".format(args.out, args.model, args.cross)
        PATH_train = os.path.join(run_name, "model", "model_last.pth")
        torch.save(model.state_dict(), PATH_train)

        if np.mean(dsc) >= sample:
            sample = np.mean(dsc)
            # PATH_dsc ="{}_{}_{}/model/model_bestdsc.pth".format(args.out, args.model, args.cross)
            PATH_dsc = os.path.join(run_name, "model", "model_bestdsc.pth")
            torch.save(model.state_dict(), PATH_dsc)

        if loss_train < sample_loss:
           sample_loss = loss_train
        #    PATH_loss ="{}_{}_{}/model/model_bestloss.pth".format(args.out, args.model, args.cross)
           PATH_loss = os.path.join(run_name, "model", "model_bestloss.pth")
